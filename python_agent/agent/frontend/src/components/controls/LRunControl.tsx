...
interface LRunControlProps {
  isComputing: boolean;
  isConnected: boolean;
  handleStop: () => void;
  handleSubmit: () => void;
  tabKey: string | null;
  setTabKey: (key: string | null) => void;
  runConfig: RunConfig;
  setRunConfig: (updater: (draft: RunConfig) => void) => void;
  runConfigDefault: RunConfig;
}

const LRunControl: React.FC<LRunControlProps> = ({
  isComputing,
  isConnected,
  handleStop,
  handleSubmit,
  ...
  const [psType, setPsType] = useState<string>("gh");
  const [psInputValue, setPsInputValue] = useState<string>("");
  const defaultInstallCommand = "pip install --editable .";
  const defaultPS =
    "https://github.com/marshmallow-code/marshmallow/issues/1357";

  const handlePsTypeChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  function onTabClicked(keyClicked: string | null) {
