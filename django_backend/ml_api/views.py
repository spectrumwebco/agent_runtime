...
try:
    ...
    from models.api.ml_infrastructure_api_models import FineTuningJobCreate
...
class MLModelViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if not ML_CLIENT_AVAILABLE:
            return Response(
                {"error": "ML Infrastructure client not available"},
...
class FineTuningJobViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        try:
            ...
            serialized_jobs = [
                FineTuningJobDetailSerializer.from_pydantic(job).data
                for job in jobs]
    ...
    def cancel(self, request, pk=None):
        ...
        try:
            self.client.cancel_fine_tuning_job(pk)  # pragma: no cover
            return Response(
                {"status": "success", "message": f"Job {pk} cancelled"})
        except Exception as e:
            return Response({"error": str(e)},
                            status=status.HTTP_400_BAD_REQUEST)
