...
type NodeRuntimeAdapter struct {
...
func NewNodeRuntimeAdapter() *NodeRuntimeAdapter {
...
func (a *NodeRuntimeAdapter) Initialize(ctx context.Context, config *RuntimeAdapterConfig) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *NodeRuntimeAdapter) Connect(ctx context.Context) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if err != nil {
		return fmt.Errorf("failed to connect to Node.js runtime API: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("Node.js runtime API health check failed with status code: %d", resp.StatusCode)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to connect to Node.js runtime WebSocket: %w", err)
	}
...
func (a *NodeRuntimeAdapter) Disconnect(ctx context.Context) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *NodeRuntimeAdapter) IsConnected() bool {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
func (a *NodeRuntimeAdapter) ExecuteTask(ctx context.Context, task *Task) (*TaskResult, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	payload := map[string]interface{}{
		...
		"runtime_type": string(RuntimeTypeNode),
	}
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("failed to execute task in Node.js runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *NodeRuntimeAdapter) GetState(ctx context.Context, key string) (interface{}, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("failed to get state from Node.js runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *NodeRuntimeAdapter) SetState(ctx context.Context, key string, value interface{}) error {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("failed to set state in Node.js runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *NodeRuntimeAdapter) SubscribeToEvents(ctx context.Context, eventType string, handler EventHandler) (string, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if _, ok := a.eventHandlers[eventType]; !ok {
		...
		resp, err := a.httpClient.Do(req)
		if err != nil {
			return "", fmt.Errorf("failed to subscribe to events in Node.js runtime: %w", err)
		}
		defer resp.Body.Close()
...
func (a *NodeRuntimeAdapter) UnsubscribeFromEvents(ctx context.Context, subscriptionID string) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if len(a.eventHandlers[eventType]) == 0 {
		...
		resp, err := a.httpClient.Do(req)
		if err != nil {
			return fmt.Errorf("failed to unsubscribe from events in Node.js runtime: %w", err)
		}
		defer resp.Body.Close()
...
func (a *NodeRuntimeAdapter) PublishEvent(ctx context.Context, event *Event) error {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	payload := map[string]interface{}{
		...
		"data":      event.Data,
		"runtime":   string(RuntimeTypeNode),
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("failed to publish event to Node.js runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *NodeRuntimeAdapter) listenForEvents() {
	for {
		select {
		case <-a.ctx.Done():
			return
