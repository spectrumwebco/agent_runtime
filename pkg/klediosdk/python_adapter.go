...
func NewPythonRuntimeAdapter() *PythonRuntimeAdapter {
	ctx, cancel := context.WithCancel(context.Background())
	return &PythonRuntimeAdapter{
...
func (a *PythonRuntimeAdapter) Initialize(ctx context.Context, config *RuntimeAdapterConfig) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *PythonRuntimeAdapter) Connect(ctx context.Context) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if err != nil {
		return fmt.Errorf("failed to connect to Python runtime API: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("Python runtime API health check failed with status code: %d", resp.StatusCode)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to connect to Python runtime WebSocket: %w", err)
	}
...
func (a *PythonRuntimeAdapter) Disconnect(ctx context.Context) error {
	a.mu.Lock()
	defer a.mu.Unlock()
...
func (a *PythonRuntimeAdapter) IsConnected() bool {
	a.mu.RLock()
	defer a.mu.RUnlock()
...
func (a *PythonRuntimeAdapter) ExecuteTask(ctx context.Context, task *Task) (*TaskResult, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	payload := map[string]interface{}{
		...
		"runtime_type": string(RuntimeTypePython),
	}
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("failed to execute task in Python runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *PythonRuntimeAdapter) GetState(ctx context.Context, key string) (interface{}, error) {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("failed to get state from Python runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *PythonRuntimeAdapter) SetState(ctx context.Context, key string, value interface{}) error {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("failed to set state in Python runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *PythonRuntimeAdapter) SubscribeToEvents(ctx context.Context, eventType string, handler EventHandler) (string, error) {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if _, ok := a.eventHandlers[eventType]; !ok {
		...
		resp, err := a.httpClient.Do(req)
		if err != nil {
			return "", fmt.Errorf("failed to subscribe to events in Python runtime: %w", err)
		}
		defer resp.Body.Close()
...
func (a *PythonRuntimeAdapter) UnsubscribeFromEvents(ctx context.Context, subscriptionID string) error {
	a.mu.Lock()
	defer a.mu.Unlock()
	...
	if len(a.eventHandlers[eventType]) == 0 {
		...
		resp, err := a.httpClient.Do(req)
		if err != nil {
			return fmt.Errorf("failed to unsubscribe from events in Python runtime: %w", err)
		}
		defer resp.Body.Close()
...
func (a *PythonRuntimeAdapter) PublishEvent(ctx context.Context, event *Event) error {
	a.mu.RLock()
	defer a.mu.RUnlock()
	...
	payload := map[string]interface{}{
		...
		"data":      event.Data,
		"runtime":   string(RuntimeTypePython),
	...
	resp, err := a.httpClient.Do(req)
	if err != nil {
		return fmt.Errorf("failed to publish event to Python runtime: %w", err)
	}
	defer resp.Body.Close()
...
func (a *PythonRuntimeAdapter) listenForEvents() {
	for {
		select {
		case <-a.ctx.Done():
			return
