...
class StaticAnalysisTool:
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized static analysis tool with {len(self.tools)} analyzers")
    ...
    def analyze(self, repository: str, branch: str, files: List[str]) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Performing static analysis on {len(files)} files in {repository}:{branch}")
        ...
        logger.info(f"Static analysis complete with {results['summary']['total_findings']} findings")
