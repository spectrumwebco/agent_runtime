...
class TrajectoryEvaluator:
    ...
    def evaluate_trajectory(self, predicted_trajectory: List[Dict[str, Any]]) -> Dict[str, float]:
...
class SWEAgentEvaluator:
    ...
    def evaluate_model(
        ...
        for task in tasks:
            ...
            prediction = next(
                (p for p in model_predictions if p.get("task_id") == task_id), 
                None
            )
        ...
        pass_rate = sum(1 for r in task_results if r.get("passed", False)) / len(task_results)
    ...
    def _evaluate_task(
        ...
        if task_type == "code_generation":
            return self._evaluate_code_generation(expected_output, predicted_output, trajectory)
        elif task_type == "code_repair":
            return self._evaluate_code_repair(expected_output, predicted_output, trajectory)
        elif task_type == "issue_resolution":
            return self._evaluate_issue_resolution(expected_output, predicted_output, trajectory)
        else:
            return {
                "passed": False,
                "score": 0.0,
    ...
    def _evaluate_code_generation(
        self, expected: Dict[str, Any], predicted: Dict[str, Any], trajectory: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    def _evaluate_code_repair(
        ...
        score = 0.2 * float(compiles) + 0.3 * float(tests_pass) + 0.5 * float(issue_fixed)
    ...
    def _calculate_code_similarity(self, expected_code: str, predicted_code: str) -> float:
...
def calculate_metrics(
    ...
    references: List[Any],
    task_type: str = "issue_resolution"
    ...
    if task_type == "issue_resolution":
        ...
        gitops_preds = [p for p, r in zip(predictions, references) if "gitops" in r.get("tags", [])]
        gitops_refs = [r for r in references if "gitops" in r.get("tags", [])]
        
        terraform_preds = [p for p, r in zip(predictions, references) if "terraform" in r.get("tags", [])]
        terraform_refs = [r for r in references if "terraform" in r.get("tags", [])]
        
        kubernetes_preds = [p for p, r in zip(predictions, references) if "kubernetes" in r.get("tags", [])]
        kubernetes_refs = [r for r in references if "kubernetes" in r.get("tags", [])]
        
        if gitops_refs:
            gitops_correct = sum(1 for p in gitops_preds if p.get("solution_correct", False))
            metrics.gitops_accuracy = gitops_correct / len(gitops_refs)
        
        if terraform_refs:
            terraform_correct = sum(1 for p in terraform_preds if p.get("solution_correct", False))
            metrics.terraform_accuracy = terraform_correct / len(terraform_refs)
        
        if kubernetes_refs:
            kubernetes_correct = sum(1 for p in kubernetes_preds if p.get("solution_correct", False))
            metrics.kubernetes_accuracy = kubernetes_correct / len(kubernetes_refs)
        
        trajectory_evaluator = TrajectoryEvaluator([r.get("trajectory", []) for r in references])
