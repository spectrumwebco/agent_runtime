...
class GiteeScraper:
    ...
    async def scrape_repositories(
            ...
            if languages:
                for language in languages:
                    ...
                    if isinstance(response, list):
                        ...
                        self.logger.info(f"Found {len(response)} repositories for {topic} in {language}")
            else:
        ...
        repositories = [repo for repo in repositories if repo.get("stargazers_count", 0) >= min_stars]
    ...
    async def scrape_issues(
        ...
        for repo in repositories:
            ...
            if not owner or not name:
                self.logger.warning(f"Skipping repository with missing owner or name: {repo}")
                continue
